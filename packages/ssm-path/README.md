...
handler(event, context, (_, response) => {
  t.is(process.env.SOME_PREFIX_CONNECTION_STRING,'some-connection-string') // The '/dev/db' path contains the CONNECTION_STRING parameter
  t.is(process.env.SOME_ACCESS_TOKEN,'some-access-token')
})
```
...
handler(event, context, (_, response) => {
  t.is(context.SOME_ACCESS_TOKEN,'some-access-token')
})
```
