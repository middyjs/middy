...
describe("@middy/glue-schema-registry", () => {
	test.afterEach((t) => {
		t.mock.reset();
		clearCache();
	});
	...
	test("resolveSchemaVersion rejects non-string schemaVersionId", async () => {
		await rejects(
			...
			(e) => {
				...
				strictEqual(
					e.message,
					"resolveSchemaVersion: schemaVersionId required",
				);
				return true;
			},
		);
	...
	test("validateOptions enforces setToContext must be a boolean", () => {
		doesNotThrow(() =>
			glueSchemaRegistryValidateOptions({ setToContext: true }),
		);
		throws(
	...
	test("resolveSchemaVersion sends command with the exact schemaVersionId", async () => {
		...
		deepStrictEqual(
			mock.commandCalls(GetSchemaVersionCommand)[0].args[0].input,
			{
				SchemaVersionId: exactId,
			},
		);
