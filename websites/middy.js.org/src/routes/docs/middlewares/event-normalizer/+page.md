...
- A record missing the fields its event source promises (a DynamoDB record without `dynamodb`, an S3 record without `s3`, an SNS record without `Sns`, a Kafka event without `records`, ...) fails the invocation with a 422 `HttpError`. `cause.data` carries `{ reason: 'Malformed event record', eventSource, message }`. This also applies to records nested in an SQS or SNS message.
