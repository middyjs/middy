...
- `contextKey` (string) (default `ssm-extension`): The key under `context.middyContext` used when `setToContext` is `true`. Override it to run two instances side by side.
