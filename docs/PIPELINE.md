...
This document describes middy's CI/CD pipeline: branch flow, what every workflow does, and how each piece maps to the controls in [docs/SPVS-COMPLIANCE.md](SPVS-COMPLIANCE.md). It addresses OWASP SPVS core control V1.3.1; control IDs below reference the SPVS 1.0 core catalogue.
...
| [test-mutation.yml](../.github/workflows/test-mutation.yml) | PR | Stryker mutation testing, one matrix leg per package, rolled up into the single `Tests (mutation)` required check | V2.2.5 |
...
There is no bypass switch for release verification. `npm audit signatures`, `gh attestation verify`, and the `npm-publish` environment approval are encoded in [release.yml](../.github/workflows/release.yml); the only way to waive any of them is to change that file, which itself requires a PR into `main` with 2 approving reviews + CODEOWNERS + the zizmor/CodeQL code-scanning gates. If an emergency ever requires such a waiver:

1. The PR title carries `[break-glass]` and links an open incident issue or security advisory.
2. The waiver is reverted in the first PR after the emergency ends.
3. The event is recorded in the incident log in [docs/INCIDENT-RESPONSE.md](INCIDENT-RESPONSE.md) with a post-incident review per its Phase 5.
